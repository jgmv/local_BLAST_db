...
- update_blast_nt.sh: Automatically downloads the BLAST nt database from the
    NCBI FTP server (ftp://ftp.ncbi.nlm.nih.gov/blast/db/). The log with the
    updated dates can be found at: $HOME/databases/blast/blast_updates.log
...
- update_blast_sra.sh: Prepares a local database with selected SRA objects
    selected. Selected objects can be added to the list within the script,
    where "# select SRA objects to download".  
